    ...
    public class WiFiTestApp : App<F7Micro, WiFiTestApp>
    {
        ...
        public async Task RunAsync()
        {
            if (NetworkName == null||NetworkPassword == null)
                throw new Exception($"Please set the {NetworkName} and {NetworkPassword} before running");

            var rgbPwmLed = new RgbPwmLed(Device, Device.Pins.OnboardLedRed, Device.Pins.OnboardLedGreen, Device.Pins.OnboardLedBlue);
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    await Device.InitWiFiAdapter().ConfigureAwait(false);
                    Console.WriteLine("Initialized WiFi adapter, scanning for networks.");
                    foreach (var network in Device.WiFiAdapter.Scan())
                    {
                        Console.WriteLine($"Found {network.Ssid}, signal strength: {network.SignalDbStrength}dB");
                    }

                    Console.WriteLine($"Connecting to {NetworkName}...");
                    ConnectionStatus connectionStatus;
                    while ((connectionStatus = (await Device.WiFiAdapter.Connect(NetworkName, NetworkPassword).ConfigureAwait(false)).ConnectionStatus) != ConnectionStatus.Success)
                    {
                    ...
                    var weather = await client.GetStringAsync("http://wttr.in/?format=%l%20%t%20%C").ConfigureAwait(false);
